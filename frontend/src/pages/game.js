...
function Example({ $app, initialState }) {
  ...
  function handleMultipleKeys() {
    ...
    if (keyState.ArrowLeft) {
      if (modeChange == false) movePaddle2WithKeyboard(1);
    }
  ...
  function handleKeyDown(event) {
    ...
    if (event.code === "KeyW" || event.code === "ArrowUp") {
      if (
        shouldPerformAction(event.code) &&
        !isSpaceBarPressed &&
        !isPastPaddle1() &&
        !isPastPaddle2()
      ) {
  ...
  function movePaddle1WithKeyboard(direction) {
    ...
    paddle1.position.x = Math.max(
      ...
      Math.min(FIELD_WIDTH / 2 - PADDLE_WIDTH / 2, newPaddleX)
    );
